...
parser.add_argument(
    "--wavefields", type=str, required=True, help="Path to the dir with the wavefiels"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    required=True,
    help="Number of hops in the iterations to save the file",
)
parser.add_argument(
    "--name",
    type=str,
    required=False,
    default="wavefield",
    help="Name of the video to be generated",
)
...
for i in range(args.iterations):
    ...
    if i % args.hop == 0:
        ...
        file_name = "%swavefield-iter-%d-grid-%d-%d.txt" % (
            args.wavefields,
            i,
            args.n1,
            args.n2,
...
out = cv2.VideoWriter(
    "{}.avi".format(args.name), cv2.VideoWriter_fourcc(*"DIVX"), 30, size
)
