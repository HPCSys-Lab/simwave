...
def accuracy(Lx, Lz, tf, freq, vp, rho, rho_z, h, p, dt=None, density=True):
    ...
    print(f"time step dt = {time_model.dt}")
    if dt is not None:
        time_model.dt = dt
    # initial grid
    ...
    # numerical_values = u_full[:, pt_x, pt_z] / h ** 2
    ...
    # compare solutions
    # return np.linalg.norm(u_full - u_analytic) / np.sqrt(u_full.size)
    # return np.linalg.norm(numerical_values - analytic_values) / np.sqrt(numerical_values.size)
...
if __name__ == "__main__":
    ...
    for order in orders:
        ...
        for spacing in spacings:
                ...
                accs[order][spacing] = accuracy(Lx_, Lz_, tf_, freq_, vp_, rho_, rho_z_, spacing, order, density=False)
