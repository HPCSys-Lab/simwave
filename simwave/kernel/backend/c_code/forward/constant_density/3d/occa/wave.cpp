...
extern "C" double forward(f_type *u, f_type *velocity, f_type *damp,
    ...
    kernel_properties["defines/BLOCK_SIZE_Y"] = BLOCK_SIZE_Y;
    ...
    occa::kernel boundary_conditions_3 = device.buildKernel(okl_file_path + "boundary_conditions_3.okl", "boundary_conditions_3", kernel_properties);
    ...
    for(size_t n = begin_timestep; n <= end_timestep; n++) {
        ...
        stencil(
            stencil_radius, nz, nx, ny, domain_size, prev_t, current_t, next_t,
            dzSquared, dxSquared, dySquared, dtSquared, dt,
            d_u, d_velocity, d_coeff, d_damp
        );

        // device kernel for section 2: add the source term        
        source_injection(
            num_sources, n, wavelet_count, next_t, nx, ny, domain_size, dtSquared,
        ...
        */
        ...
        boundary_conditions_1(stencil_radius, nz, nx, ny, y_before, y_after, next_t, domain_size, d_u);

        // device kernel for section 3.2
        // boundary conditions on the front and back (fixed on X)
        boundary_conditions_2(stencil_radius, nz, nx, ny, x_before, x_after, next_t, domain_size, d_u);

        // device kernel for section 3.3
        // boundary conditions on the bottom and top (fixed on Z)
        boundary_conditions_3(stencil_radius, nz, nx, ny, z_before, z_after, next_t, domain_size, d_u);

        // device kernel for section 4: compute the receivers
        sismogram(
            num_receivers, n, current_t, nx, ny, domain_size, 
        ...
        if(saving_stride > 1){
            ...
            if(n % saving_stride == 1){
                ...
                if(saving_stride % 2 == 0 && n < end_timestep){
                    ...
                    swap_grid_in_even_stride(nz, nx, ny, domain_size, current_t, next_t, d_u);
