        ...
        */
        ...
        for(size_t i = stencil_radius; i < nz - stencil_radius; i++) {
            for(size_t j = stencil_radius; j < nx - stencil_radius; j++) {
                ...
                float z1 = ((prev_base[current + nx] - prev_base[current]) * (density[current + nx] + density[current])) / density[current + nx];
                float z2 = ((prev_base[current] - prev_base[current - nx]) * (density[current] + density[current - nx])) / density[current - nx];
                float term_z = (z1 - z2) / (2 * dzSquared);

                //nominator with damp coefficient
                float nominator = (1.0 + damp[current] * dt);

                float value = dtSquared * vel_base[current] * vel_base[current] * (term_z + term_x) / nominator;
