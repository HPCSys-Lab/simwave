...
double gradient(f_type *u, f_type *v, f_type *grad, f_type *velocity, f_type *damp,
   ...
   for(size_t n = end_timestep; n >= begin_timestep; n--) {
        ...
        */
        ...
        #ifdef GPU_OPENACC
        #pragma acc parallel loop collapse(3) present(coeff,damp,v,velocity)
        #endif
        ...
        for(size_t i = stencil_radius; i < nz - stencil_radius; i++) {
            for(size_t j = stencil_radius; j < nx - stencil_radius; j++) {
                for(size_t k = stencil_radius; k < ny - stencil_radius; k++) {
                    ...
                    f_type sum_y = coeff[0] * v[current_snapshot];
                    ...
                    for(size_t ir = 1; ir <= stencil_radius; ir++){
                        ...
                        sum_y += coeff[ir] * (v[current_snapshot + ir] + v[current_snapshot - ir]);

                        //neighbors in the X direction
                        sum_x += coeff[ir] * (v[current_snapshot + (ir * ny)] + v[current_snapshot - (ir * ny)]);

                        //neighbors in the Z direction
                        sum_z += coeff[ir] * (v[current_snapshot + (ir * nx * ny)] + v[current_snapshot - (ir * nx * ny)]);
                    }
