...
// gradient_3D_variable_density
double gradient(f_type *u, f_type *v, f_type *grad, f_type *velocity, f_type *density, f_type *damp,
    ...
    for(size_t n = end_timestep; n >= begin_timestep; n--) {
        ...
        */
        ...
        #ifdef GPU_OPENACC
        #pragma acc parallel loop collapse(3) present(coeff_order1,coeff_order2,damp,v,velocity,density)
        #endif
        ...
        for(size_t i = stencil_radius; i < nz - stencil_radius; i++) {
            for(size_t j = stencil_radius; j < nx - stencil_radius; j++) {
                for(size_t k = stencil_radius; k < ny - stencil_radius; k++) {
                    ...
                    f_type sd_pressure_y = coeff_order2[0] * v[current_snapshot];
                    f_type sd_pressure_x = coeff_order2[0] * v[current_snapshot];
                    f_type sd_pressure_z = coeff_order2[0] * v[current_snapshot];
                    ...
                    for(size_t ir = 1; ir <= stencil_radius; ir++){
                        ...
                        sd_pressure_y += coeff_order2[ir] * (v[current_snapshot + ir] + v[current_snapshot - ir]);
                        fd_pressure_y += coeff_order1[ir] * (v[current_snapshot + ir] - v[current_snapshot - ir]);
                        fd_density_y += coeff_order1[ir] * (density[domain_offset + ir] - density[domain_offset - ir]);

                        //neighbors in the X direction
                        sd_pressure_x += coeff_order2[ir] * (v[current_snapshot + (ir * ny)] + v[current_snapshot - (ir * ny)]);
                        fd_pressure_x += coeff_order1[ir] * (v[current_snapshot + (ir * nx)] - v[current_snapshot - (ir * nx)]);
                        fd_density_x += coeff_order1[ir] * (density[domain_offset + (ir * nx)] - density[domain_offset - (ir * nx)]);

                        //neighbors in the Z direction
                        sd_pressure_z += coeff_order2[ir] * (v[current_snapshot + (ir * nx * ny)] + v[current_snapshot - (ir * nx * ny)]);
                        fd_pressure_z += coeff_order1[ir] * (v[current_snapshot + (ir * nx * ny)] - v[current_snapshot - (ir * nx * ny)]);
        ...
        */
        ...
        #ifdef GPU_OPENACC
        #pragma acc parallel loop collapse(2) present(v)
        #endif
            ...
            for(size_t k = stencil_radius; k < ny - stencil_radius; k++){
                ...
                if(x_before == 2){
                    ...
                    for(size_t ir = 1; ir <= stencil_radius; ir++){
                        size_t domain_offset = (i * nx + stencil_radius) * ny + k;
                        size_t next_snapshot = next_t * domain_size + domain_offset;
                        v[next_snapshot - (ir * ny)] = v[next_snapshot + (ir * ny)];
                    }
                }
            ...
            for(size_t k = stencil_radius; k < ny - stencil_radius; k++){
                ...
                if(z_before == 2){
                    ...
                    for(size_t ir = 1; ir <= stencil_radius; ir++){
                        size_t domain_offset = (stencil_radius * nx + j) * ny + k;
                        size_t next_snapshot = next_t * domain_size + domain_offset;
                        v[next_snapshot - (ir * nx * ny)] = v[next_snapshot + (ir * nx * ny)];
                    }
                }
        ...
        */
        ...
        #ifdef GPU_OPENACC
        #pragma acc parallel loop collapse(3) present(v,u,grad)
        #endif
