...
double gradient(f_type *u, f_type *v, f_type *grad, f_type *velocity, f_type *density, f_type *damp,
    ...
    for(size_t n = end_timestep; n >= begin_timestep; n--) {
        ...
        */
        ...
        #ifdef GPU_OPENACC
        #pragma acc parallel loop collapse(2) present(coeff_order1,coeff_order2,damp,v,velocity,density)
        #endif
        ...
        for(size_t i = stencil_radius; i < nz - stencil_radius; i++) {
            for(size_t j = stencil_radius; j < nx - stencil_radius; j++) {
                ...
                for(size_t ir = 1; ir <= stencil_radius; ir++){
                    ...
                    sd_pressure_x += coeff_order2[ir] * (v[current_snapshot + ir] + v[current_snapshot - ir]);
                    fd_pressure_x += coeff_order1[ir] * (v[current_snapshot + ir] - v[current_snapshot - ir]);
                    fd_density_x += coeff_order1[ir] * (density[domain_offset + ir] - density[domain_offset - ir]);

                    //neighbors in the vertical direction
                    sd_pressure_z += coeff_order2[ir] * (v[current_snapshot + (ir * nx)] + v[current_snapshot - (ir * nx)]);
                    fd_pressure_z += coeff_order1[ir] * (v[current_snapshot + (ir * nx)] - v[current_snapshot - (ir * nx)]);
