...
class Model:
    ...
    """
    def __init__(self, velocity_model: np.ndarray, grid_spacing: Tuple[float, float, float],
                 dt: float, num_timesteps: int, space_order: Optional[int] = 2,
    ...
    def grid_spacing(self, value: Tuple[float, float, float]):
        ...
        self._grid_spacing = (self.dtype(value[0]), self.dtype(value[1]), self.dtype(value[2]))
    ...
    def critical_dt(self) -> float:
        ...
        fd_coeffs = findiff.coefficients(deriv=2, acc=self.space_order)['center']['coefficients']
    ...
    def stencil_coefficients(self) -> np.ndarray:
        ...
        coeffs = findiff.coefficients(deriv=2, acc=self.space_order)['center']['coefficients']
