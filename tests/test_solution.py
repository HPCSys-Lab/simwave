...
def accuracy(spacing, bbox, order, dt, t0, tf, c, f0, src, rec):
    ...
    source, receiver, wavelet = _acquisition(space_model, time_model, src, rec, f0)
    ...
    # analytical solution
    u_exact = analytical_solution(space_model, f0, src[0], rec).flatten()[:time_model.timesteps]
    # compare solutions
...
@pytest.mark.parametrize(
    ...
    [((0.5, 0.5), (-40, 440, -40, 440), 8, 0.1, 0, 150.075, 1.5, 0.09, [(200, 200)], [(260, 260)], 1e-4)]
)
...
@pytest.mark.parametrize(
    "spacing, bbox, order, timesteps, t0, tf, c, f0, src, rec, time_convergence",
    [((0.5, 0.5), (-40, 440, -40, 440), 8, [0.1, 0.075, 0.04, 0.025], 0, 150.075, 1.5, 0.09, [(200, 200)], [(260, 260)], 1.7)]
)
def test_convergence_in_time(spacing, bbox, order, timesteps, t0, tf, c, f0, src, rec, time_convergence):
    accs = []
    for dt in timesteps:
         accs.append(accuracy(spacing, bbox, order, dt, t0, tf, c, f0, src, rec))
    assert np.poly1d(np.polyfit(np.log(timesteps), np.log(accs), 1))[1] > time_convergence

@pytest.mark.parametrize(
    "spacings, bbox, space_orders, dt, t0, tf, c, f0, src, rec, space_rates",
    [([2.0, 2.5, 4.0], (-40, 440, -40, 440), [2, 4, 6, 8, 10], 0.025, 0, 150.075, 1.5, 0.09, [(200, 200)], [(260, 260)], [1.7, 4, 6, 7.7, 8.7])]
)
def test_convergence_in_space(spacings, bbox, space_orders, dt, t0, tf, c, f0, src, rec, space_rates):
    accs = {}
    conv_rates = []
    for order in space_orders:
        accs[order] = {}
        for spacing in spacings:
            accs[order][spacing] = accuracy(tuple([spacing, spacing]), bbox, order, dt, t0, tf, c, f0, src, rec)
        step = list(accs[order].keys())
        error = list(accs[order].values())
        conv_rates.append(np.poly1d(np.polyfit(np.log(step), np.log(error), 1))[1])
